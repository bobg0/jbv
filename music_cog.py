...
class music_cog(commands.Cog):
    ...
    async def play_next(self):
        if len(self.music_queue) > 0:
            ...
            song = data['url']
            print("song:", song)
            self.vc.play(discord.FFmpegPCMAudio(song, executable= "/opt/homebrew/Cellar/ffmpeg/7.1_3/bin/ffmpeg", **self.FFMPEG_OPTIONS), after=lambda e: asyncio.run_coroutine_threadsafe(self.play_next(), self.bot.loop))
        else:
            self.is_playing = False
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            try:
                ...
                print(e, "could not play the song")
        else:
            self.is_playing = False
